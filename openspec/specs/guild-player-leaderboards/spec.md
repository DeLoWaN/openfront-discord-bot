...
guild clan-tag prefixes, and MAY render the linked-versus-observed state as an
inline indicator within the player cell instead of a standalone column.
...
- **THEN** the leaderboard reorders the same guild-scoped player rows by that
  metric and shows donation-focused columns
